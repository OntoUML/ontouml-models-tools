...
def run_data_quality_verifications(arguments: dict):
    ...
    for dataset_number, dataset_name in enumerate(datasets_list):
        ...
        if problems_char_list:
            ...
            LOGGER.warning(f"Dataset {dataset_name} has {len(problems_char_list)} ProblemChar case(s).")
        if problems_ends_list:
            append_problems_output_ends_file(dataset_name, problems_ends_list)
            LOGGER.warning(f"Dataset {dataset_name} has {len(problems_ends_list)} ProblemEnds case(s).")
        ...
        if problems_ster_list:
            ...
            LOGGER.warning(f"Dataset {dataset_name} has {len(problems_ster_list)} ProblemOldStereotypes case(s).")
